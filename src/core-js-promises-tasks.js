...
function getPromise(number) {
  return new Promise(function getProm(resolve, reject) {
    if (number >= 0) {
      resolve();
    } else {
      reject();
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((promiseArr) => {
    return promiseArr.map((promise) => {
...
function queuePromises(promises) {
  return promises.reduce(async (accumulator, current) => {
